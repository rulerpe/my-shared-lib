 ...
 */
export async function _setMeta(key, data) {
 ...
 */
export async function purge(filter) {
// exports.purge = async function (filter) {
    ...
    const metaArray = await Promise.all(keys.map((k) => getMeta(k)));
    const meta = _.object(keys, metaArray);

    return Promise.all(keys.filter((k) => filter(k, meta[k])).map(remove));
};
